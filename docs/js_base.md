...
简而言之 一条`__proto__`连起来的链条，递归访问`__proto__`必须最终到头，并且值是null。
...
闭包会引起内存泄露，这时就需要垃圾回收机制。JS的垃圾回收的方法主要有2种。标记清除法和引用计数 具体见[垃圾回收机制](/browser.html#垃圾回收机制)文章
...
|:-:|:-|
|*|匹配前面的子表达式任意，相当于{0,}|
|+|匹配前面的子表达式大于等于一次，相当于{1,}|
|?|匹配前面的子表达式零次或者一次，相当于{0,1}|
|.|匹配除“\n”和"\r"之外的任何单个字符|
